# C-Store KPI Dashboard (Full App with Price Ladder + Store Map)
# - Safe session_state defaults (no post-instantiation writes)
# - Unique widget keys
# - Robust "All" multiselect
# - KPI, Trends, Top-N, Basket Affinity
...
freq = st.sidebar.radio("Time Granularity", ["Daily", "Weekly", "Monthly"], index=["Daily","Weekly","Monthly"].index(st.session_state["global_freq"]), horizontal=True, key="global_freq_widget")
...
def calculate_trends(df: pd.DataFrame, rule: str, group_dim: str | None) -> pd.DataFrame:
...
def calculate_affinity_rules(df: pd.DataFrame, key_col: str) -> pd.DataFrame:
    ...
    def qty_in_txids(item: str, tx_ids: set[str]) -> float:
...
def display_kpi_trends(df: pd.DataFrame, rule: str, selected_stores, selected_categories, selected_brands, selected_products):
    ...
    metric = st.selectbox("Select Metric", ["Total_Sale", "Quantity", "Spend per Basket", "Transactions"], index=0, key="trend_metric")
...
def display_price_ladder(df: pd.DataFrame):
    ...
    if ladder_level == "Item":
        ...
        tmp = df.copy()
        # small jitter on x to de-overlap
